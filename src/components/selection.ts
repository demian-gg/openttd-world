...
function renderSelectionBorder(
  ...
  for (const [startX, startY, endX, endY] of edges) {
    drawDashedLine(context, startX + 1, startY + 1, endX + 1, endY + 1, "rgba(0, 0, 0, 0.5)");
  }

  // Draw borders.
  for (const [startX, startY, endX, endY] of edges) {
    drawDashedLine(context, startX, startY, endX, endY, borderColor);
  }
}
...
function renderSelectionCutout(
  ...
  renderSelectionBorder(context, bounds, borderColor);
}
...
export const { init: initSelectionComponent } = defineComponent<ComponentProps>(
  {
    ...
    render(context: RenderContext) {
      ...
      if (mode === "pan") {
        if (!bounds) return;
        context.save();
        context.globalAlpha = PAN_MODE_SELECTION_OPACITY;
        renderSelectionBorder(context, bounds, borderColor);
        context.restore();
        return;
      }
      ...
      if (bounds) {
        renderSelectionCutout(context, bounds, borderColor);
      }
