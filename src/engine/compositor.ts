...
function compositeFrame(): void {
  const { context, backgroundColor } = getEngineState();
  ...
  for (const [layerId, group] of byLayer) {
    ...
    if (layer.dirty) {
      ...
      for (const component of group) {
        component.render(layer.context);
      }
  ...
  for (const layer of layers) {
    ...
    const destX = Math.round((context.canvas.width - destWidth) / 2) + layer.x;
    const destY = Math.round((context.canvas.height - destHeight) / 2) + layer.y;

    // Draw layer content to main canvas.
    context.save();
    context.globalAlpha = layer.opacity;
    context.globalCompositeOperation = layer.blendMode;
    context.drawImage(layer.canvas, destX, destY, destWidth, destHeight);
    context.restore();
